...
static inline void rintd_vec(double *src, double *dst, int len)
{
    ...
    for (; (i = VSETVL64(len)) > 0; len -= i) {
        ...
        b = VCVT_INT_DOUBLE(VCVT_RNE_DOUBLE_INT(a, i), i);
...
static inline void roundd_vec(double *src, double *dst, int len)
{
    ...
    for (; (i = VSETVL64(len)) > 0; len -= i) {
        V_ELT_DOUBLE a, b;
        a = VLOAD_DOUBLE(src_tmp, i);
        b = VCVT_INT_DOUBLE(VCVT_RNA_DOUBLE_INT(a, i), i);
        VSTORE_DOUBLE(dst_tmp, b, i);
        src_tmp += i;
        dst_tmp += i;
    }
...
static inline void ceild_vec(double *src, double *dst, int len)
{
    ...
    for (; (i = VSETVL64(len)) > 0; len -= i) {
        V_ELT_DOUBLE a, b;
        a = VLOAD_DOUBLE(src_tmp, i);
        b = VCVT_INT_DOUBLE(VCVT_RINF_DOUBLE_INT(a, i), i);
        VSTORE_DOUBLE(dst_tmp, b, i);
        src_tmp += i;
        dst_tmp += i;
    }
...
static inline void floord_vec(double *src, double *dst, int len)
{
    ...
    for (; (i = VSETVL64(len)) > 0; len -= i) {
        V_ELT_DOUBLE a, b;
        a = VLOAD_DOUBLE(src_tmp, i);
        b = VCVT_INT_DOUBLE(VCVT_RMINF_DOUBLE_INT(a, i), i);
        VSTORE_DOUBLE(dst_tmp, b, i);
        src_tmp += i;
        dst_tmp += i;
    }
...
static inline void truncd_vec(double *src, double *dst, int len)
{
    ...
    for (; (i = VSETVL64(len)) > 0; len -= i) {
        V_ELT_DOUBLE a, b;
        a = VLOAD_DOUBLE(src_tmp, i);
        b = VCVT_INT_DOUBLE(VCVT_RTZ_DOUBLE_INT(a, i), i);
        VSTORE_DOUBLE(dst_tmp, b, i);
        src_tmp += i;
        dst_tmp += i;
    }
...
static inline V_ELT_DOUBLEH atan2_pd(V_ELT_DOUBLEH y, V_ELT_DOUBLEH x, V_ELT_DOUBLEH ATAN_P1_vec, V_ELT_DOUBLEH ATAN_P2_vec, V_ELT_DOUBLEH ATAN_P3_vec, V_ELT_DOUBLEH ATAN_P4_vec, V_ELT_DOUBLEH ATAN_Q1_vec, V_ELT_DOUBLEH ATAN_Q2_vec, V_ELT_DOUBLEH ATAN_Q3_vec, V_ELT_DOUBLEH ATAN_Q4_vec, V_ELT_DOUBLEH MOREBITS_vec, V_ELT_DOUBLEH PIO4_vec, V_ELT_DOUBLEH min1_vec, size_t i)
{
    V_ELT_DOUBLEH z, w;
                                   ...
                                   size_t i)
{
    ...
    _MM_SET_ROUNDING_MODE(_MM_ROUND_TOWARD_ZERO);
#endif
    n = VCVT_RMINF_DOUBLEH_INTH(px, i);
#ifdef NO_RTZ
    _MM_SET_ROUNDING_MODE(reg_ori);
#endif
    px = VCVT_INTH_DOUBLEH(n, i);
